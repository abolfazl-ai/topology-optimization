...
class TopologyOptimizer:
    ...
    def optimizeDesign(self, maxEpochs, minEpochs):
        ...
        for epoch in range(maxEpochs):
            ...
            if (epoch % 20 == 0):
                titleStr = "Iter {:d} , Obj {:.2F} , vol {:.2F}".format(epoch, self.objective.item() * self.obj0,
                                                                        currentVolumeFraction)
                self.Pltr.plotDensity(self.xy.detach().cpu().numpy(), rho_np.reshape((self.FE.nelx, self.FE.nely)),
                                      titleStr)
                print(titleStr)
        ...
        titleStr = "Iter {:d} , Obj {:.2F} , vol {:.2F}".format(epoch, self.objective.item() * self.obj0,
                                                                currentVolumeFraction)
        self.Pltr.plotDensity(self.xyPlot.detach().cpu().numpy(), rho_np.reshape(
            (self.FE.nelx * self.boundaryResolution, self.FE.nely * self.boundaryResolution)), titleStr)
