...
class FE:
    ...
    def initializeRectangularGeometry(self, mesh, bc):
        ...
        if (self.physics == self.PHYSICS_OPTIONS['Structural']):
            ...
            self.edofMat = np.matlib.repmat(edofVec, 4, 1).T + np.matlib.repmat(
                np.array([0, self.nely + 1, self.nely, -1]), self.nelx * self.nely, 1)
        ...
        self.bb_xmin, self.bb_xmax, self.bb_ymin, self.bb_ymax = 0., self.nelx * self.elemSize[0], 0., self.nely * \
                                                                     self.elemSize[1]
